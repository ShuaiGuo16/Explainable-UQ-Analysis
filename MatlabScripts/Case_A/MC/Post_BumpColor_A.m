...
for i = 1:ele_FR
    ...
    for j = 1:ele_GR
        ...
        count(i,j) = points_count(area_range,f_MC_GR(1:30000,3),f_MC_FR(1:30000,3))/30000;
...
figure(1)
for i = 1:30000
    if f_MC_FR(i,3)<min(Ran_FR) || f_MC_FR(i,3)>max(Ran_FR) || f_MC_GR(i,3)<min(Ran_GR) || f_MC_GR(i,3)>max(Ran_GR)
        continue
    else
        location_i = ceil((f_MC_FR(i,3)-min(Ran_FR))/int_FR);
        location_j = ceil((f_MC_GR(i,3)-min(Ran_GR))/int_GR);
        r = (count(location_i,location_j)-min_density)/(max_density-min_density)*(0.05-0.8)+0.8;
        color_data = [r r r];
        plot(f_MC_GR(i,3),f_MC_FR(i,3),'o','LineWidth',1,'MarkerFaceColor',color_data,'MarkerEdgeColor',color_data)
        hold on
    end
    
    if rem(i,1000) == 0
        
        remain_num = 30 - i/1000
