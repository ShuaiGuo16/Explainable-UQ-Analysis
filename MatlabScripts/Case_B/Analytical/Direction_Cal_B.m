function [ a_omega_value, b_omega_value ] = Direction_Cal_B( freq_value, growth_value )
...
a_omega = diff(real(H_subs),freq);
b_omega = diff(imag(H_subs),freq);
a_omega_value = double(subs(subs(a_omega,freq,freq_value),growth,growth_value));
b_omega_value = double(subs(subs(b_omega,freq,freq_value),growth,growth_value));
