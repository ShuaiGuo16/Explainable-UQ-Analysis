% Test: to check when all sig=0 points collapes together

clear
clc

load 'Datasample_00.mat'   
temp_mean = Datasample_00(10,:);      % Randomly select one sample
...
%% Calculate H expression
H = solve(CharEqn==0,F);

%% Calculate Acoustic Term
A = -1/H;

%% Calculate FTF
F = 0;
for k = 1:16
    F = F + temp_mean(k)*exp(-omega*1i*(k+2)*delta_Ts);
end

%% Calculate OpenLoop Functiom
A_OLTF = subs(A,omega,freq-0*1i);
FTF_OLTF = subs(F,omega,freq-0*1i);

target_expression = A_OLTF*FTF_OLTF;
slope_exp = diff(target_expression,freq);
slope = double(subs(slope_exp,freq,FR_ref))
