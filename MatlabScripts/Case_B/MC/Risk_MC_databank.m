...
f_MC_GR = zeros(sample_num,1);
f_MC_FR = zeros(sample_num,1);
...
for index = 1:16
    Samples(:,index) = Samples_St(:,index)*variance_translation(index) + mean_translation(index);
end
% ----------------------------------------------------------------------------------
...
initial_value = [107.6*2*pi,-4;];                         % Modes based on derterministic analzsis
...
for index=1:sample_num
    ...
    h = Samples(index,:);
        ...
        f_MC_FR(index) = real(Eigen)/(2*pi);
        f_MC_GR(index) = -imag(Eigen);
...
% ------------------------------------------------------------------------------------
figure(2)
histogram(f_MC_GR,'Normalization','pdf')

figure(3)
plot(f_MC_GR,f_MC_FR,'o')
